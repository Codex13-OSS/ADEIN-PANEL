...
-- ============================================================
-- ADEIN 006 · UNIFIED BUSINESS + PUBLICATION DOMAIN
-- Canonical lineage: 004 -> 005 -> 006
-- 001 is preserved as historical lineage; its non-overlapping
-- business DDL is incorporated above for convergent environments.
-- properties.status remains operational inventory state.
-- publication_status is an independent editorial/web state.
-- ============================================================
...
CREATE TABLE IF NOT EXISTS property_listings (
  ...
  price_mode VARCHAR(40) NOT NULL DEFAULT 'amount',
  price_amount DECIMAL(14,2) NULL,
  currency VARCHAR(10) NOT NULL DEFAULT 'MXN',
  price_display_override VARCHAR(120) NULL,
  commercial_badge VARCHAR(100) NULL,
  ...
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  UNIQUE KEY uq_property_listings_property (property_id),
  UNIQUE KEY uq_property_listings_slug (slug),
  KEY idx_property_listings_publication (publication_status, display_order),
  KEY idx_property_listings_location (location_key),
  KEY idx_property_listings_type (property_type),
  CONSTRAINT chk_property_listings_status CHECK (publication_status IN ('draft','published','unpublished')),
  CONSTRAINT fk_property_listings_property FOREIGN KEY (property_id) REFERENCES properties(id)
    ON UPDATE CASCADE ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

CREATE TABLE IF NOT EXISTS property_listing_operations (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  UNIQUE KEY uq_property_listing_operations (listing_id, operation_code),
  KEY idx_property_listing_operations_code (operation_code),
  CONSTRAINT fk_property_listing_operations_listing FOREIGN KEY (listing_id) REFERENCES property_listings(id)
    ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS property_listing_features (
  ...
  feature_value VARCHAR(220) NULL,
  sort_order INT NOT NULL DEFAULT 0,
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  UNIQUE KEY uq_property_listing_features_key (listing_id, feature_key),
  KEY idx_property_listing_features_order (listing_id, sort_order),
  CONSTRAINT fk_property_listing_features_listing FOREIGN KEY (listing_id) REFERENCES property_listings(id)
    ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS property_listing_images (
  ...
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  UNIQUE KEY uq_property_listing_images_storage (listing_id, storage_key),
  KEY idx_property_listing_images_order (listing_id, sort_order),
  KEY idx_property_listing_images_cover (listing_id, is_cover),
  CONSTRAINT chk_property_listing_images_cover CHECK (is_cover IN (0,1)),
  CONSTRAINT fk_property_listing_images_listing FOREIGN KEY (listing_id) REFERENCES property_listings(id)
    ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
