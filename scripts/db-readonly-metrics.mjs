...
async function run() {
  ...
  try {
    ...
    const [followupsByStatusRows] = await connection.query(
      'SELECT status, COUNT(*) AS total FROM crm_followups GROUP BY status'
    );
    ...
    const [pipelineRows] = await connection.query(
      `SELECT
