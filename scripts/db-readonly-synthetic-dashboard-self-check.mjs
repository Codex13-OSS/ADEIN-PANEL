...
checks.no_synthetic_token_column_assumption = !endpointSource.includes('synthetic_token');
...
checks.uses_placeholders = endpointSource.includes('LIKE ?');
checks.uses_real_columns = ['name', 'lot_code', 'full_name', 'contract_code', 'notes', 'raw_payload_json'].every((c) => endpointSource.includes(`'${c}'`));
