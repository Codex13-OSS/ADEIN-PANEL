...
const failures = []; const assertions = {}; let payload;

try { payload = JSON.parse(run.stdout || '{}'); assertions.jsonParseable = true; } catch { assertions.jsonParseable = false; failures.push('Output is not JSON parseable'); }
assertions.exitZero = run.status === 0; if (!assertions.exitZero) failures.push(`Script exited with code ${run.status}`);
if (assertions.jsonParseable) {
  ...
  assertions.relationshipsValidated = ['property->client','lot->property','contract->client','contract->lot','payment_schedule->contract'].every((name) => rel.some((r) => r.relation === name && r.ok === true));
  ...
  const dbAttempt = spawnSync(process.execPath, [scriptPath], { encoding: 'utf8', env: { ...process.env, ADEIN_BP_REHEARSAL_DB_MODE: 'rollback_only' } });
  ...
  Object.entries(assertions).forEach(([k, v]) => { if (!v) failures.push(`assertion_failed:${k}`); });
}
const ok = failures.length === 0; if (!ok) process.exitCode = 1;
