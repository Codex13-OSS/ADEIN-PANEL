...
export default function PropertiesAdminPage({ token }: Props) {
  ...
  return <div className="properties-page properties-admin-v2 properties-admin-simplified">
    <section className="properties-toolbar-card"><div><span className="properties-eyebrow">Inventario inmobiliario</span><h2>Propiedades</h2><p>Administra la información que usa tu equipo y la ficha que verán tus clientes.</p></div><div className="properties-kpis"><div><strong>{properties.length}</strong><span>Propiedades</span></div><div><strong>{properties.filter((item) => item.status === 'active').length}</strong><span>Activas</span></div><button type="button" className="properties-primary-action" onClick={() => setShowCreate((value) => !value)}>+ Nueva propiedad</button></div></section>
    {message && <div className="properties-message" role="status">{message}</div>}
    {showCreate && <section className="properties-card new-property-panel"><div><span className="properties-eyebrow">Nueva propiedad</span><h3>Agrega una propiedad al inventario</h3><p className="properties-help">Comienza con la información que usará tu equipo para identificarla.</p></div><form className="properties-form" onSubmit={createProperty}><label>Nombre para identificarla en ADEIN<small>Sólo lo verá tu equipo.</small><input required maxLength={180} value={newProperty.name} onChange={(e) => setNewProperty({ ...newProperty, name: e.target.value })} placeholder="Escribe el nombre de la propiedad" /></label><label>Ubicación interna<small>Ayuda a tu equipo a organizar y encontrar la propiedad.</small><input maxLength={220} value={newProperty.location} onChange={(e) => setNewProperty({ ...newProperty, location: e.target.value })} placeholder="Zona, municipio o referencia" /></label><button disabled={busy}>Crear propiedad</button></form></section>}
    <div className="properties-admin-layout">
      <aside className="properties-card properties-selector"><div className="properties-card-heading"><div><span className="properties-eyebrow">Inventario</span><h3>Selecciona una propiedad</h3><p>Elige una ficha para consultar o editar su información.</p></div></div><div className="properties-list">{properties.map((property) => <button type="button" key={property.id} className={`property-row ${selectedId === property.id ? 'selected' : ''}`} onClick={() => setSelectedId(property.id)}><div><strong>{property.name}</strong><span>{property.location || 'Ubicación por definir'}</span></div><div className="property-row-meta"><span className={`inventory-status status-${property.status}`}>{property.status === 'active' ? 'Activa' : 'Inactiva'}</span><small>{property.lotCount} lotes</small></div></button>)}{!properties.length && <div className="properties-empty">Aún no hay propiedades.</div>}</div></aside>
      <main className="property-editor">{selected ? <>
        <header className="properties-card property-editor-header"><div><span className="properties-eyebrow">Propiedad seleccionada</span><h3>{selected.name}</h3><p>{selected.location || 'Ubicación interna por definir'} · {money(selected.minAvailablePrice)}</p></div><span className={`inventory-status status-${listing?.publicationStatus || 'draft'}`}>{statusLabel}</span></header>
        <section className="properties-card editor-section"><div className="editor-section-title"><div><h4>Datos de la propiedad</h4><p>Información interna para organizar el inventario.</p></div></div><div className="editor-grid three"><label>Nombre para identificarla en ADEIN<small>Sólo lo verá tu equipo.</small><input value={general.name} onChange={(e) => setGeneral({ ...general, name: e.target.value })} /></label><label>Ubicación interna<small>Sirve para la organización interna de tu equipo.</small><input value={general.location} onChange={(e) => setGeneral({ ...general, location: e.target.value })} /></label><label>Disponibilidad<small>Define si la propiedad está activa en el inventario.</small><select value={general.status} onChange={(e) => setGeneral({ ...general, status: e.target.value })}><option value="active">Activa</option><option value="inactive">Inactiva</option></select></label></div><div className="section-actions"><button type="button" onClick={saveGeneral} disabled={busy}>Guardar datos de la propiedad</button></div></section>
        <section className="properties-card editor-section"><div className="editor-section-title"><div><h4>Información que verá el cliente</h4><p>Esta información aparecerá en el catálogo público de ADEIN.</p></div></div><div className="editor-grid two"><label className="wide">Nombre que verá el cliente<input value={draft.title} onChange={(e) => setDraft({ ...draft, title: e.target.value })} placeholder="Escribe un nombre claro para la publicación" /></label><label>Tipo de propiedad<select value={draft.propertyType} onChange={(e) => setDraft({ ...draft, propertyType: e.target.value })}><option value="terreno">Terreno</option><option value="finca">Finca</option></select></label><label>Ubicación que verá el cliente<input value={draft.location} onChange={(e) => setDraft({ ...draft, location: e.target.value, locationKey: slugify(e.target.value) })} placeholder="Escribe la zona o ubicación pública" /></label><label className="wide">Descripción<textarea rows={5} value={draft.description} onChange={(e) => setDraft({ ...draft, description: e.target.value })} placeholder="Describe la ubicación, cualidades y ventajas de la propiedad." /></label></div><details className="advanced-options"><summary>Opciones avanzadas</summary><p className="properties-help">Estos ajustes son opcionales y normalmente no necesitan cambios.</p><div className="editor-grid three"><label>Clave de ubicación<input value={draft.locationKey} onChange={(e) => setDraft({ ...draft, locationKey: slugify(e.target.value) })} placeholder="Clave para el catálogo" /><small>ADEIN la genera automáticamente. Normalmente no necesitas modificarla.</small></label><label>Etiqueta destacada (opcional)<input value={draft.badge} onChange={(e) => setDraft({ ...draft, badge: e.target.value })} placeholder="Escribe una etiqueta" /><small>Muestra una etiqueta especial en el catálogo.</small></label><label>Orden en el catálogo<input type="number" min="0" value={draft.displayOrder} onChange={(e) => setDraft({ ...draft, displayOrder: e.target.value })} /></label></div></details></section>
        <section className="properties-card editor-section"><div className="editor-section-title"><div><h4>Precio</h4><p>Define cómo se mostrarán las condiciones comerciales.</p></div></div><div className="editor-grid three price-grid"><label>Operación<select value={draft.operation} onChange={(e) => setDraft({ ...draft, operation: e.target.value })}><option value="venta">Venta</option><option value="renta">Renta</option></select></label><label>Forma de mostrar precio<select value={draft.priceMode} onChange={(e) => setDraft({ ...draft, priceMode: e.target.value })}><option value="amount">Precio fijo</option><option value="from">Desde</option><option value="consult">Consultar</option></select></label><label>Precio<input type="number" min="0" step="0.01" value={draft.price} onChange={(e) => setDraft({ ...draft, price: e.target.value })} placeholder="Escribe el precio" /></label><label>Moneda<select value={draft.currency} onChange={(e) => setDraft({ ...draft, currency: e.target.value })}><option value="MXN">MXN</option><option value="USD">USD</option></select></label><label className="wide">Texto público opcional<input value={draft.priceDisplay} onChange={(e) => setDraft({ ...draft, priceDisplay: e.target.value })} placeholder="Escribe cómo deseas mostrar el precio" /></label></div></section>
        <section className="properties-card editor-section"><div className="editor-section-title"><div><h4>Características</h4><p>Agrega datos que ayuden al cliente a entender la propiedad, como superficie, medidas, servicios o escrituras.</p></div></div>{listing ? <div className="feature-editor-list">{listing.features.map((feature, index) => <div className="feature-editor-row" key={feature.id}><div className="feature-order-controls"><button disabled={busy || index === 0} onClick={() => moveFeature(index, -1)} aria-label="Subir característica">↑</button><button disabled={busy || index === listing.features.length - 1} onClick={() => moveFeature(index, 1)} aria-label="Bajar característica">↓</button></div><label><span>Característica</span><input value={featureDrafts[feature.id]?.label || ''} onChange={(e) => setFeatureDrafts({ ...featureDrafts, [feature.id]: { ...featureDrafts[feature.id], label: e.target.value } })} aria-label="Característica" /></label><label><span>Detalle</span><input value={featureDrafts[feature.id]?.value || ''} onChange={(e) => setFeatureDrafts({ ...featureDrafts, [feature.id]: { ...featureDrafts[feature.id], value: e.target.value } })} aria-label="Detalle" /></label><button className="text-action" onClick={() => updateFeature(feature.id)}>Guardar</button><button className="danger-action" onClick={() => deleteFeature(feature.id)}>Eliminar</button></div>)}{!listing.features.length && <div className="properties-empty">Aún no hay características.</div>}<div className="feature-add-row"><label><span>Característica</span><input value={newFeature.label} onChange={(e) => setNewFeature({ ...newFeature, label: e.target.value })} placeholder="Escribe una característica" /></label><label><span>Detalle</span><input value={newFeature.value} onChange={(e) => setNewFeature({ ...newFeature, value: e.target.value })} placeholder="Escribe el detalle" /></label><button disabled={busy || !newFeature.label.trim()} onClick={addFeature}>Agregar característica</button></div></div> : <div className="properties-empty">Guarda primero la información pública para agregar características.</div>}</section>
        <section className="properties-card editor-section photos-section"><div className="editor-section-title"><div><h4>Fotografías</h4><p>Sube y ordena las imágenes que verá el cliente. La portada se muestra primero en el catálogo.</p></div></div>{listing ? <><label className="photo-upload"><input type="file" multiple accept="image/jpeg,image/png,image/webp" onChange={uploadImages} disabled={busy} /><strong>Seleccionar fotografías</strong><span>JPEG, JPG, PNG o WEBP · máximo 5 MB por archivo</span></label>{uploadStatus && <p className="upload-status">{uploadStatus}</p>}<div className="photo-grid">{listing.images.map((image, index) => <article className={`photo-card ${image.isCover ? 'cover' : ''}`} key={image.id}>{previews[image.id] ? <img src={previews[image.id]} alt={image.altText || 'Fotografía de propiedad'} /> : <div className="photo-loading">Cargando vista previa…</div>}{image.isCover && <span className="cover-badge">Portada actual</span>}<div className="photo-card-actions"><button disabled={busy || index === 0} onClick={() => moveImage(index, -1)} aria-label="Mover foto a la izquierda">←</button><button disabled={busy || index === listing.images.length - 1} onClick={() => moveImage(index, 1)} aria-label="Mover foto a la derecha">→</button>{!image.isCover && <button onClick={() => setCover(image.id)}>Usar como portada</button>}<button className="danger-action" onClick={() => deleteImage(image.id)}>Eliminar</button></div></article>)}{!listing.images.length && <div className="properties-empty photo-empty">Aún no hay fotografías. Selecciona archivos para comenzar.</div>}</div></> : <div className="properties-empty">Guarda primero la ficha pública para subir fotografías.</div>}</section>
        <section className="properties-card editor-section publication-section"><div className="editor-section-title"><div><h4>Publicación</h4><p>Revisa la ficha y decide cuándo debe aparecer en el catálogo.</p></div></div>{general.status === 'inactive' && <div className="publication-warning">Esta propiedad está inactiva. Aunque la ficha figure como publicada, no será visible públicamente.</div>}<div className="publication-footer"><div><span className={`inventory-status status-${listing?.publicationStatus || 'draft'}`}>{statusLabel}</span><p>{listing ? `${listing.images.length} fotografías · ${listing.features.length} características` : 'La ficha pública todavía no se ha creado.'}</p></div><div className="publication-actions"><button type="button" className="properties-primary-action" onClick={() => saveListing()} disabled={busy || !draft.title.trim()}>{listing ? "Guardar cambios" : "Crear ficha pública"}</button>{listing?.publicationStatus === 'published' ? <button type="button" className="properties-secondary-action" onClick={() => saveListing('unpublished')} disabled={busy}>Despublicar</button> : <button type="button" onClick={() => saveListing('published')} disabled={busy || !draft.title.trim()}>Publicar</button>}</div></div></section>
